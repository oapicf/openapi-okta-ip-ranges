    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
    ...
    async fn ip_ranges_json_get(
        &self,
        context: &C) -> Result<IpRangesJsonGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<std::collections::HashMap<String, models::IpRangesJsonGet200ResponseValue>>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(IpRangesJsonGetResponse::AJSONObjectOfRegionalCellsWithIPRanges
                    (body)
                )
            }
            code => {
