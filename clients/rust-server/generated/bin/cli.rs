...
#[structopt(
    ...
    version = "0.9.0-pre.0",
    about = "CLI access to OpenAPI Okta IP Ranges"
)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    let result = match args.operation {
        Operation::IpRangesJsonGet {
            ...
            info!("Performing a IpRangesJsonGet request");

            let result = client.ip_ranges_json_get(
            ).await?;
            debug!("Result: {:?}", result);
            ...
            match result {
                ...
                (body)
                => "AJSONObjectOfRegionalCellsWithIPRanges\n".to_string()
                   +
