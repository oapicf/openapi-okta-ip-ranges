...
mod paths {
    ...
    lazy_static! {
        pub static ref GLOBAL_REGEX_SET: regex::RegexSet = regex::RegexSet::new(vec![
            r"^/okta-ip-ranges/ip_ranges.json$"
        ])
        .expect("Unable to create global regex set");
    }
    pub(crate) static ID_IP_RANGES_JSON: usize = 0;
}
    ...
    ReqBody::Data: Send,
{
    ...
    fn call(&self, req: (Request<ReqBody>, C)) -> Self::Future {
            ...
            ReqBody::Data: Send,
        {
            ...
            // IpRangesJsonGet - GET /ip_ranges.json
            hyper::Method::GET if path.matched(paths::ID_IP_RANGES_JSON) => {
                handle_ip_ranges_json_get(api_impl, uri, headers, body, context, validation).await
            },

            _ if path.matched(paths::ID_IP_RANGES_JSON) => method_not_allowed(),
...
async fn handle_ip_ranges_json_get<T, C, ReqBody>(
    mut api_impl: T,
    uri: hyper::Uri,
    headers: HeaderMap,
    body: ReqBody,
...
where
    ...
    ReqBody::Data: Send,
{
                                let result = api_impl.ip_ranges_json_get(
                                        &context
                                    ).await;
                                ...
                                response.headers_mut().insert(
                                        ...
                                        match result {
                                            Ok(rsp) => match rsp {
                                                IpRangesJsonGetResponse::AJSONObjectOfRegionalCellsWithIPRanges
                                                    (body)
                                                => {
...
impl<T> RequestParser<T> for ApiRequestParser {
    fn parse_operation_id(request: &Request<T>) -> Option<&'static str> {
        ...
        match *request.method() {
            // IpRangesJsonGet - GET /ip_ranges.json
            hyper::Method::GET if path.matched(paths::ID_IP_RANGES_JSON) => Some("IpRangesJsonGet"),
            _ => None,
        }
    }
}
